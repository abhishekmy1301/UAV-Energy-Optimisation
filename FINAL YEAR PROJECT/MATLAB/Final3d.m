...
%MST
figure('Name','Minimum Spanning Tree with Minimum Weight Pairs','NumberTitle','off');


G=graph(distmat_mst);
T=minspantree(G,'Method','sparse'); %sparse=Kruskal,dense=prims
plotNodes(nodes);
...
% i = 1;
% while i <= length(eulerian_graph)
%     n1 = eulerian_graph(i, 1);
%     n2 = eulerian_graph(i, 2);
%     j = 1;
%     while j <= length(eulerian_graph)
%         if (j ~= i) && ((n1 == eulerian_graph(j, 1) && n2 == eulerian_graph(j, 2)) || (n1 == eulerian_graph(j, 2) && n2 == eulerian_graph(j, 1)))
%             eulerian_graph(j, :) = [];
%             i = i - 1;
%             break;
%         end
...
% visited=eulerian_graph(:,3); min_node=0;min_edge=0;
% 
% i=1;
% 
% while all(visited>=0)
...
% 
%         if visited(j)>0 
%             if(eulerian_graph(j,1)==current_node ||eulerian_graph(j,2)==current_node)
%                 if dist(nodes(eulerian_graph(j,1),:),nodes(eulerian_graph(j,2),:))<min_dist
%                     min_dist=dist(nodes(eulerian_graph(j,1),:),nodes(eulerian_graph(j,2),:));
%                     if(current_node==eulerian_graph(j,1))
%                         min_node=eulerian_graph(j,2); min_edge=j;
%                     end
% 
%                     if(current_node==eulerian_graph(j,2))
%                         min_node=eulerian_graph(j,1); min_edge=j;
%                     end
%                 end
%             end
%         end
...
% 
% 
% visited(min_edge)=visited(min_edge)-1;
% 
% eulerian_path(p)=min_node;p=p+1;
% current_node=min_node;
% 
% %        if(current_node==min_node)
% %         eulerian_path(p)=eulerian_path(p-2);
% %         visited(eulerian_path(p))=false;
% %     
% %         current_node=eulerian_path(p);
% %     
% %         elseif current_node~=min_node
% %         current_node=min_node;
% %     end
% i=i+1;
% 
