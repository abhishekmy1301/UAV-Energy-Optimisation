...
N=70; %Number of nodes excluding the origin
rng(0) % Could be any random natural number
seed=rng;
...
%         if(cost_wh>max_cluster_cost)
%             st=60.*ones(N+1,1); % service time of UAV at each node in seconds
%             st(1)=0; % service time at depot is 0s
%             max_cluster_cost=cost_wh(getCost(nodes,cluster_path),3,st);
%         end
...
function cost_wh=cost_wh(cp_cost,V,st)
%    cp=path; %chosen path
% cp_cost=getCost(nodes,cp);%

P_T=power_uav(V); % power to travel at V m/s

cost_wh=((cp_cost*1000/3)/3600);
cost_wh=P_T*cost_wh;
%cost_wh=cost_wh+sum(st)*15/3600; %15 Joules
end
