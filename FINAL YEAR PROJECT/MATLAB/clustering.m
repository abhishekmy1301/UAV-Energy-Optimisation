clc;clear;close all;
tic
final_cost_wh=zeros(3,10,2); % third dimensions stores number of clusters
tr=1;
for N=100:100 %Number of nodes excluding the origin
rng(14) % Could be any random natural number
seed=rng;
...
figure('Name',str,NumberTitle='on');
while cost_wh(max_cluster_cost,3,m1)>capacity  
...
disp(final_cost);
%fprintf("Final Cost in Wh = %f\n",cost_wh(final_cost,3,m1));

final_cost_wh(1,tr,1)=cost_wh(final_cost,3,m1);
final_cost_wh(1,tr,2)=k-1;
% CLUSTERING
capacity=174.6*2; %Wh
m2=4.57;
max_cluster_cost=inf; %Wh
...
str="K-Means - Number of clusters: "+num2str(k);
figure('Name',str,NumberTitle='on');
while cost_wh(max_cluster_cost,3,m2)>capacity  
...
disp(final_cost);
%fprintf("Final Cost in Wh = %f\n",cost_wh(final_cost,3,m2));

final_cost_wh(2,tr,1)=cost_wh(final_cost,3,m2);
final_cost_wh(2,tr,2)=k-1;

capacity=129.96*6; %Wh
m3=6.600;
max_cluster_cost=inf; %Wh
k=1;
str="K-Means - Number of clusters: "+num2str(k);
figure('Name',str,NumberTitle='on');
while cost_wh(max_cluster_cost,3,m3)>capacity  
...
disp(final_cost);
%fprintf("Final Cost in Wh = %f\n",cost_wh(final_cost,3,m3));
final_cost_wh(3,tr,1)=cost_wh(final_cost,3,m3);
final_cost_wh(3,tr,2)=k-1;
tr=tr+1;
end
...
omega=50;%Blade angular velocity in radians/second
...
function cost_wh=cost_wh(cp_cost,V,m)
%    cp=path; %chosen path
% cp_cost=getCost(nodes,cp);%

P_T=power_uav(V,m); % power to travel at V m/s

total_st=(cp_cost*1500/3)/3600;
